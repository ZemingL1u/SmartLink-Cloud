...
func (e *SysLoc) getAllChildLocIDs(parentID int) ([]int, error) {
	var locIDs []int

	// 查询当前节点的未删除子节点
	subLocations := make([]models.SysLoc, 0)
	if err := e.Orm.Model(&models.SysLoc{}).Where("parent_id = ? AND deleted_at IS NULL", parentID).Find(&subLocations).Error; err != nil {
		e.Log.Errorf("db error: %s", err)
		return nil, err
	}
	...
	for _, loc := range subLocations {
		locIDs = append(locIDs, loc.LocId)

		// 递归查询子节点的子节点 ID
		childLocIDs, err := e.getAllChildLocIDs(loc.LocId)
		if err != nil {
			return nil, err
		}
		// 这里我们只添加子节点的 ID，而不是所有后代的 ID
		locIDs = append(locIDs, childLocIDs...)
	}
