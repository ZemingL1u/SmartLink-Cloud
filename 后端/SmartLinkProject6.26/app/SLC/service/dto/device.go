...
type DeviceGetPageReq struct {
	...
	ImageSrc       string `form:"imageSrc" search:"type:contains;column:image_src;table:device" comment:"图片地址"`
	DLocJoin       `search:"type:left;on:loc_id:loc_id;table:device;join:sys_loc"`
	DTypeJoin      `search:"type:left;on:type_id:type_id;table:device;join:device_type"`
	DeviceOrder
}

type DeviceGetPageReqByDeviceIds struct {
	dto.Pagination `search:"-"`
	DeviceIds      []int `json:"deviceIds" binding:"required"` // 设备ID列表
}
...
type DeviceInsertReq struct {
	...
	ImageSrc   string `json:"imageSrc" comment:"图片地址"`                       // 图片地址
}
