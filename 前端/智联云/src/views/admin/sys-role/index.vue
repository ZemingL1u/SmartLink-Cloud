  ...
  <BasicLayout>
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-table
          v-loading="loading"
          :data="indexedRoleList"
          border
          @selection-change="handleSelectionChange"
          @sort-change="handleSortChang"
        >
          <el-table-column type="selection" width="70" align="center" />
          <el-table-column label="编号" width="150" align="center" prop="编号" sortable="custom" />
          <!-- <el-table-column label="编码" sortable="custom" prop="roleId" width="80" /> -->
          <el-table-column label="名称" sortable="custom" align="center" prop="roleName" :show-overflow-tooltip="true" />
          <el-table-column label="权限字符" align="center" prop="roleKey" :show-overflow-tooltip="true" width="200" />
          <!-- <el-table-column label="排序" sortable="custom" prop="roleSort" width="80" /> -->
          <el-table-column label="状态" align="center" sortable="custom" width="150">
          ...
          <el-table-column label="角色权限"  align="center"prop="remark" width="400"/>
          <el-table-column
            label="操作"
            align="left"
...
export default {
  ...
  methods: {
    ...
    submitDataScope: function() {
      if (this.form.roleId !== undefined) {
        this.form.locIds = this.getDeptAllCheckedKeys()
    ...
    handleExport() {
      this.$confirm('是否确认导出所有角色数据项?', '警告', {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['角色编号', '角色名称', '权限字符', '显示顺序', '状态', '角色权限']
  ...
  computed: {
    ...
    indexedRoleList() {
      // 根据列表数据和当前分页的索引计算序号
      const { roleList, queryParams: { pageIndex, pageSize } } = this;
      return roleList.map((item, index) => {
        return {
          ...item, // 保留原有数据
        编号: (pageIndex - 1) * pageSize + index + 1 // 计算序号
        };
      });
    }
  }
