  ...
  <BasicLayout>
    <template #wrapper>
      <el-card class="box-card">
        ...
        >
          <el-table-column prop="locName" label="区域名称" align="center" width="300" />
          <el-table-column prop="sort" label="排序" align="center" width="150" />
          <el-table-column prop="status" label="状态" align="center" :formatter="statusFormat" width="150">
          ...
          <el-table-column label="负责人" align="center" prop="leader" width="150">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button
                ...
                @click="navigateToLargeScreen(scope.row)"
              >大屏
              </el-button>
              <el-button
        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" :close-on-click-modal="false">
          <el-form ref="form" :model="form" :rules="rules" label-width="80px">
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="区域名称" prop="locName">
                  <el-input v-model="form.locName" placeholder="请输入区域名称" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
  ...
  methods: {
    ...
    getTreeselect(e) {
      getDeptList().then(response => {
        ...
        if (e === 'update') {
          const loc = { locId: 0, locName: '主类目', children: [], isDisabled: true }
          loc.children = response.data
          this.locOptions.push(loc)
        } else {
          const loc = { locId: 0, locName: '主类目', children: [] }
          loc.children = response.data
          this.locOptions.push(loc)
        }
      })
    ...
    handleUpdate(row) {
      ...
      getDept(row.locId).then(response => {
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.locId !== undefined) {
            updateDept(this.form, this.form.locId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const Ids = (row.locId && [row.locId]) || this.ids
      this.$confirm(
        '是否确认删除名称为"' + row.locName + '"的数据项?',
        '警告',
        {
...
@/api/admin/sys-loc@/api/admin/sys-loc
